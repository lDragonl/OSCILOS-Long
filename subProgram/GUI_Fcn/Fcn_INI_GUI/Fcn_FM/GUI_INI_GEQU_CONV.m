...
handles.HP.GEQU_CONV.nb_points = 51; % Number of points used for discretisation along r
...
handles.HP.GEQU_CONV.CFL   = 0.3; % in m/s, this is a vector if there are multple flame.
handles.HP.GEQU_CONV.F_Cutoff = 500; % in m/s, this is a vector if there are multple flame.
...
handles.HP.GEQU_CONV.nb_points = str2double(get(handles.edit_GEQU_CONV_a7,'String')); % str2num required here to be able to deal with vector inputs
handles.HP.GEQU_CONV.nf_points = str2double(get(handles.edit_GEQU_CONV_a8,'String')); % str2num required here to be able to deal with vector inputs
...
dx1         = (handles.HP.GEQU_CONV.rb-handles.HP.GEQU_CONV.ra)/(handles.HP.GEQU_CONV.nb_points-1);
dx2         = handles.HP.GEQU_CONV.UC/handles.HP.GEQU_CONV.F_Cutoff/30;
dx          = min(dx1,dx2);
if(dx1 > dx2)
    disp('The number of points chosen are not enough to resolve the maximum frequency chosen');
    disp(['Grid spacing reduced from ',num2str(dx1),' to ',num2str(dx2)]);
end

if (mod(handles.HP.GEQU_CONV.ra,dx)~=0)
    disp('The burner location must be at a grid corner.');
    disp(['Grid spacing changed from ',num2str(dx),'...']);
    dx = handles.HP.GEQU_CONV.ra/(round(handles.HP.GEQU_CONV.ra/dx));
    disp(['... to ',num2str(dx),'!']);    
end     

handles.HP.GEQU_CONV.dx = dx; % str2num required here to be able to deal with vector inputs
handles.HP.GEQU_CONV.dy = handles.HP.GEQU_CONV.dx;

if (mod(handles.HP.GEQU_CONV.rb,dx)~=0)
    disp('The end of the flame location must be at a grid corner.');
    disp(['Location of rb changed from ',num2str(handles.HP.GEQU_CONV.rb),'...']);
    handles.HP.GEQU_CONV.rb = handles.HP.GEQU_CONV.dx*round(handles.HP.GEQU_CONV.rb/handles.HP.GEQU_CONV.dx);
    disp(['... to ',num2str(handles.HP.GEQU_CONV.rb),'!']);    
end  
...
[handles.HP.GEQU_CONV.dataGhost, handles.HP.GEQU_CONV.gridGhost, ...
    handles.HP.GEQU_CONV.SET, handles.HP.GEQU_CONV.vyPrime_no_stencil] = ...
    FcnLLS_Main_INI(handles.HP.GEQU_CONV.Ugs, handles.HP.GEQU_CONV.UC, ...
    handles.HP.GEQU_CONV.SU, handles.HP.GEQU_CONV.dx, handles.HP.GEQU_CONV.dy, ...
    handles.HP.GEQU_CONV.ra, handles.HP.GEQU_CONV.rb);

 handles.HP.GEQU_CONV.gridGhost.RA = handles.HP.GEQU_CONV.ra;
...
try
    ...
    % This loop isn't really required, as each time we are only plotting one flame.
    % Still its good to keep the code general
    %plot the flame sheet
    for s = 1:length(CI.CD.indexHP(handles.HP_num))
          out = contourcs(handles.HP.GEQU_CONV.gridGhost.xs{1}(1,1+handles.HP.GEQU_CONV.gridGhost.stencil*3:end-handles.HP.GEQU_CONV.gridGhost.stencil),handles.HP.GEQU_CONV.gridGhost.xs{2}(1+handles.HP.GEQU_CONV.gridGhost.stencil:end-handles.HP.GEQU_CONV.gridGhost.stencil,1),handles.HP.GEQU_CONV.dataGhost(1+handles.HP.GEQU_CONV.gridGhost.stencil:end-handles.HP.GEQU_CONV.gridGhost.stencil,1+handles.HP.GEQU_CONV.gridGhost.stencil*3:end-handles.HP.GEQU_CONV.gridGhost.stencil),[0 0]);
          plot(1000*out.Y + 1000 *x_sample(CI.CD.indexHP(handles.HP_num(s))),1000*out.X,'r-')
          plot(1000*out.Y + 1000 *x_sample(CI.CD.indexHP(handles.HP_num(s))),-1000*out.X,'r-')
    end
    ...
    text(1000*(x_sample(end)+W.*5/32), 0, 'outlet','FontSize',15,...
        'interpreter','latex','HorizontalAlignment','center');
    text(1000*x_sample(2), 1000*2*r_sample(2),...
        'flame','FontSize',15,...
        'interpreter','latex','HorizontalAlignment','center');
