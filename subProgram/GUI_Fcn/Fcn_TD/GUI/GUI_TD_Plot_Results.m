...
% Last Modified by GUIDE v2.5 28-Jan-2015 18:40:06
...
set(handles.pb_Plot,    'units', 'points',...
    'Fontunits','points',...
    'position',[pW*1.5/3*1/10 pH*2/10 pW*2.5/10 pH*6/10],...
    'fontsize',handles.FontSize(2),...
    'string','Plot',...
    'backgroundcolor',handles.bgcolor{3});
set(handles.pb_SaveData,    'units', 'points',...
    'Fontunits','points',...
    'position',[pW*11/3*1/10 pH*2/10 pW*2.5/10 pH*6/10],...
    'fontsize',handles.FontSize(2),...
    'string','Save vars.',...
    'enable','off',...
    'backgroundcolor',handles.bgcolor{3});
set(handles.pb_Cancel,....
    'units', 'points',...
    'Fontunits','points',...
    'position',[pW*20.5/3*1/10 pH*2/10 pW*2.5/10 pH*6/10],...
    'fontsize',handles.FontSize(2),...
    'string','Cancel',...
    'backgroundcolor',handles.bgcolor{3});
%
...
if x > CI.CD.x_sample(end) || x < CI.CD.x_sample(1)
    ...
    switch popPlot
        case 1  % time evolution
            ...
            CI.TD.OUT{end_cell}.DataType = 1; % Standard x,y plot
            CI.TD.OUT{end_cell}.xLabel = xlabelStr; % store x label
            CI.TD.OUT{end_cell}.yLabel = ylabelStr; % store x label
            CI.TD.OUT{end_cell}.xData = CI.TD.tSp; % store time vector
            CI.TD.OUT{end_cell}.yData = yData; % Store yData
        case 2  % psd
            ...
            CI.TD.OUT{end_cell}.xLabel = xlabelStr; % store x label
            CI.TD.OUT{end_cell}.yLabel = ylabelStr; % store x label
            CI.TD.OUT{end_cell}.xData = PSD_plot(1,:); % store freq vector
            CI.TD.OUT{end_cell}.yData = PSD_plot(2,:); % Store yData
    end
...
% handles    structure with handles and user data (see GUIDATA)
Fcn_Plots(hObject)
set(handles.pb_SaveData,'enable','on') % enable the save data button, since data has been generated
...
global CI
Fcn_Save(CI.TD.OUT)
%clear CI.TD.OUT; CI.TD.OUT= {}; % reset the contents of the output structure


function Fcn_Save(output_vars)
%var_names = fieldnames(output_vars)

% convert the cell array to matrix, nicer display
output_vars_mat = cell2mat(output_vars);
uisave('output_vars_mat','UserData/plotted_vars.mat');
