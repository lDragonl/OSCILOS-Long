...
deltah = 1E4;
Q_mean = pi * rb^2 * rho1 * Ugs * deltah * area_ratio; % Q_mean needs to be calculated based on teh actual area of the combustor, where the flame holder is (hence the multiplication by aratio)
dt = 1E-4;
nb_iter = 1E6;
...
%     [q_ratio(runner),xi] = Fcn_TD_Gequ_interface( SU, xi, y_vec, dt, t_vec(runner), U1, area_ratio, uratio(runner),Q_mean, deltah,rho1 );
